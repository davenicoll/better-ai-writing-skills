...
metadata:
  originalAuthor: Conor Bronsdon (upstream avoid-ai-writing, synced to v3.34.0)
...
**Quotation-mark consistency (rewrite and edit).** Before rewriting, note which convention the original uses for double quotes and for apostrophes/single quotes: straight or curly, majority wins. After each rewrite, make the prose you edited match that convention before the second-pass audit or delivery. Apply this only to editable spans; quoted material, code, tables, and attributed text keep the exemptions above. If an explicit house-style quote setting exists, it overrides the inferred convention. Detect mode never runs this pass.
...
## House style (optional): `--style <guide>`

`--style` copyedits to a named house style on top of the de-AI pass (which always runs). No guides are bundled, and this repo ships no config checker, so the layer is best-effort from general knowledge rather than a verified feature.

**Named guide from memory.** If someone passes `--style "APA"` or `"Chicago"`, apply the register and mechanics you know, open with a status line such as `Applying APA from general knowledge (not verified; no compliance claim).`, and make no compliance claim. Do **not** reproduce the guide's copyrighted text, and note that your knowledge may reflect an older edition. Paywalled guides (Chicago, APA, MLA, AP) are never bundled in any form.

**How `--style` composes.** It is a third axis alongside `--voice` and `--context`, and the narrowest wins: a guide's explicit mechanics beat everything, then `--voice`, then the guide's register, then `--context`. When a guide's mechanics conflict with the AI-ism catalog the guide wins the mechanic (for example, CMOS keeps deliberate em dashes); still flag the AI *habit* such as em-dash stacking. A bare de-AI request (no `--style`) is unchanged; don't apply a guide to a genre it wasn't written for.
